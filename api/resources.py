...
from fastapi import Request, Response, APIRouter as Blueprint
...
class Resource(Blueprint):
    def __init__(self, schema: BaseSchema,
        ...
        async def show(id: str):
            return jsonify(await self.model.prisma().find_unique(
                where={'id': id}))
        ...
        async def count(field: str, value: str, operator: str = 'equals'):
            return jsonify(await self.model.prisma().count(
                where={field: {operator: value}}
            ))
            
        @self.post('/')
        async def create(schema: self.schema):
            return jsonify(await self.model.prisma().create(
                data=schema.dict(exclude_unset=True)))
            
        @self.post('/{id}')
        async def update(id: str, schema: self.schema):
            return jsonify(await self.model.prisma().upsert(
                where={'id': id},
                data={
        ...
        async def replace(id: str, schema: self.schema):
            return jsonify(await self.model.prisma().update(
                where={'id': id},
                data=schema.dict(exclude_unset=True)
            ))
        ...
        async def delete(id: str):
            return jsonify(await self.model.prisma().delete(
                where={'id': id}
            ))